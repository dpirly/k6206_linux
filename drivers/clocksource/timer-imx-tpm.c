...
static void tpm_set_mode(enum clock_event_mode mode,
	struct clock_event_device *evt)
{
	switch (mode) {
	case CLOCK_EVT_MODE_PERIODIC:
		pr_err("Periodic mode is not supported in TPM timer\n");
		break;
...
static void __init tpm_timer_init(struct device_node *np)
{
	struct clk *clk;
	uint32_t val;
	int irq;
