...
static int ci_extcon_register(struct ci_hdrc *ci)
{
	...
	id->ci = ci;
	if (!IS_ERR(id->edev)) {
		ret = extcon_register_notifier(id->edev, &id->nb);
		if (ret < 0) {
	...
	vbus->ci = ci;
	if (!IS_ERR(vbus->edev)) {
		ret = extcon_register_notifier(vbus->edev, &vbus->nb);
		if (ret < 0) {
			extcon_unregister_notifier(id->edev, &id->nb);
...
static void ci_extcon_unregister(struct ci_hdrc *ci)
{
	...
	if (!IS_ERR(cable->edev))
		extcon_unregister_notifier(cable->edev, &cable->nb);
